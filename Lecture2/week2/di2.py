...
for line in data:
    start_vertex = str(line[0])
    for end_tuple in line[1:]:
        end_vertex, length = end_tuple.split(",")
        end_vertex, length = str(end_vertex), int(length)
        init_graph[start_vertex][end_vertex] = length
...
for t in [7, 37, 59, 82, 99, 115, 133, 165, 188, 197]:
    print_result(previous_nodes, shortest_path, "1", str(t))
